...
function TeamCard({ person }: { person: TeamMember }) {
  ...
  return (
    <div
      className="group [perspective:1200px] w-full h-[460px] cursor-pointer"
      onClick={handleFlip}
    >
      <div
      ...
      >
        ...
        <div className="absolute inset-0 rounded-3xl overflow-hidden shadow-md [backface-visibility:hidden] bg-surface">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 text-surface">
            ...
            <p className="text-body text-surface/80">{role}</p>
          </div>
        </div>
        ...
        <div className="absolute inset-0 rounded-3xl bg-surface shadow-md p-6 flex flex-col justify-between [transform:rotateY(180deg)] [backface-visibility:hidden]">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-primary/5 border border-primary/10 rounded-2xl p-4 flex flex-col gap-2">
              <p className="text-xs uppercase tracking-wide text-primary font-semibold">Role</p>
          ...
          <div className="mt-6 pt-4 border-t border-border flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              />
              <div>
                <p className="text-body font-semibold text-text leading-tight">{name}</p>
                <p className="text-xs text-muted">Badminton Coach</p>
              </div>
            </div>
  ...
  return (
    <section className="bg-background py-24 px-6 flex justify-center">
      <div className="w-full max-w-7xl">
        ...
        {/* MOBILE CAROUSEL */}
        <div className="lg:hidden flex gap-6 overflow-x-auto snap-x snap-mandatory pb-6 px-2">
          {team.map((person) => (
